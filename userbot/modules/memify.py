...
async def memify(event):
    ...
    if input_file.endswith(".tgs"):
        ...
        await event.edit("**Extracting first frame...**")
        converted_file = os.path.join(TEMP_DOWNLOAD_DIRECTORY, "meme.jpg")
    ...
    try:
        final_image = await draw_meme_text(input_file, input_str)
...
async def draw_meme_text(image_path, text):
    ...
    if image_path.endswith(".mp4"):
        image_name = "memify.jpg"
    else:
        image_name = "memify.webp"

    final_image = os.path.join(TEMP_DOWNLOAD_DIRECTORY, image_name)
...
async def take_screen_shot(video_file: str,
    ...
    thumb_image_path = path or os.path.join(
        TEMP_DOWNLOAD_DIRECTORY, f"{os.path.basename(video_file)}.webp")
